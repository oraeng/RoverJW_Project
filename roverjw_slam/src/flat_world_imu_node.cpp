...
FlatWorldImuNode::FlatWorldImuNode() // 생성자로 초기화(Constructor)
{
  bool init_result = init();
  ROS_ASSERT(init_result);
}

FlatWorldImuNode::~FlatWorldImuNode() // 소멸자(destructor)
{
}

bool FlatWorldImuNode::init() // 생성자에서 publisher와 subscriber 정의하는 코드
{
...
void FlatWorldImuNode::msgCallback(const sensor_msgs::ImuConstPtr imu_in) // 메시지 불러왔을때 콜백함수. 첨엔 선형가속도 0,0,9.8로 초기화
{
...
int main(int argc, char* argv[])
{
  ...
  FlatWorldImuNode flat_world_imu_node; // flat_world_imu_node라는 객체 선언 ..
